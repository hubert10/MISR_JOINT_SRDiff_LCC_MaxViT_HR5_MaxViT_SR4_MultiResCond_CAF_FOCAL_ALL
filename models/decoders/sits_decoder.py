...
class UNetDecoder(nn.Module):
    ...
    def forward(self, res0, res1, res2, res3, h, w):
        ...
        out = self.segmentation_head(x)  # torch.Size([4, 64, 256, 256])
        # print("self.segmentation_head(x):", out.shape)

        sits_logits = F.interpolate(
            out, size=(h, w), mode="bilinear", align_corners=False
        )

        # print()
        # print("features[0]:", dec_features[0].shape)
        # print("features[1]:", dec_features[1].shape)
        # print("features[2]:", dec_features[2].shape)
        # print()
        # sits_logits: used as final classification of sits branch
        # multi_lvls_cls: used for sits branch auxiliary supervision
        # enc_features: used for CAF module between Aer and SITS sensors
